/**
 * @description: This javascript file sends the correct information for an order to the server, as the customer is signed out the customers first and last name are sent to the server.
 * @author: Tofunmi Onaeko
 * @date: 19/03/21
 * @version: 4.0
 */

window.addEventListener("load", function() {

    var submit = document.getElementById("submitButton")

    submit.addEventListener("click", function() {

        var firstname = document.getElementById("fname").value;
        var lastname = document.getElementById("lname").value;
        var address;
        var order_type;
        //In order to get the transaction amount I parse the order that has been saved to local storage and select the total that has been saved
        var transactionAmount = JSON.parse(window.localStorage.getItem('total'));
        
        //If the customer has selected home delivery then the order type is set to home delivery and their address details are added to the order file, 
        //if pickup is selected, pickup is returned, else, this area of the order object will be blank and the order will be rejected when the customer tries to pay.
        if (document.getElementById("homedelivery").checked) {
            order_type = "delivery";
            address = document.getElementById("buildingname").value + " " + document.getElementById("streetaddress").value + ", " + document.getElementById("postcode").value;
            
        } else if(document.getElementById("pickup").checked){
            order_type = "pickup";
            address = "pickup";

        } else if(document.getElementById("homedelivery1").checked){
            order_type = "delivery";
            address = document.getElementById("buildingname1").value + " " + document.getElementById("streetaddress1").value + ", " + document.getElementById("postcode1").value;

        } else if(document.getElementById("pickup1").checked){
            order_type = "pickup";
            address = "pickup";

        } else {
            order_type = "there's a problem";

        }

        //The code below creates an order object
        var order = {
            "firstname" : firstname,
            "lastname" : lastname,
            "address" : address,
            "order_type" : order_type,
            "transactionAmount" : transactionAmount
        }

        console.log(order);
        //The code below takes the order object created and creates a JSON file
        var orderSignedOut = JSON.stringify(order);

        //The code block below takes the JSON object created and sends it to the server using a post request
        http = new XMLHttpRequest();
        //url missing below
        //url = ;
        http.open("POST", url);
        http.send(orderSignedOut);

    });    

});